...
folder_feat =strcat(path,'/features/features_dim', dim, '/scale', num2str(scale_factor), '-shift', int2str(shift) );
...
for video_ts= 1: n_videos
    ...
    seqID_ts    = action_seq_names(video_ts,3);
        ...
        save_name = strcat('./FV_training/scale',num2str(scale_factor), '-shift',  int2str(shift),  '/FV_', action_name_ts, '_seq', seqID_ts, '_Ng', Ng, '.txt');
