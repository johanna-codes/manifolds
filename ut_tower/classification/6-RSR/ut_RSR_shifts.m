...
load_sub_path_train =strcat(path, 'cov_matrices/one-cov-mat/scale', int2str(scale_factor), '-shift', int2str(shift_train) );


for s=1:length(all_means_shifts)
    shift_test = vec_shift(s);
    
    load_sub_path_test =strcat(path, 'cov_matrices/one-cov-mat/scale', int2str(scale_factor), '-shift', int2str(shift_test) );
    ...
    for video_ts= 1: num_videos
        ...
        for video_tr=1: num_videos
            if (video_tr~=video_ts)
                ...
                %disp(tr);
                name_load_cov_tr = strcat( load_sub_path_train, '/cov_', action_name_tr, '_seq', seqID_tr, '.h5');
        ...
        name_load_cov_ts = strcat( load_sub_path_test, '/cov_', action_name_ts, '_seq', seqID_ts, '.h5');
