...
for s=1:length(vec_shift)
    ...
    load_sub_path_test = strcat(path, 'dim_', int2str(dim), '/cov_matrices/one-cov-mat/scale', int2str(scale_factor), '-shift', int2str(shift_test) );
    ...
    for video_ts= 1: n_videos %One Run
        ...
        if (~(strcmp(action_name,'Run-Side') && strcmp(folder_n,'001')))
            ...
            labels_test = act_ts;
            %disp('Testing with: ');
            name_load_cov_ts = strcat( load_sub_path_test, '/cov_', action_name_ts, '_', folder_n_ts, '_dim', int2str(dim), '.h5');
            ...
            CRR = RSR(TrainSet,TestSet,best_Beta,SR_Lambda_input);
