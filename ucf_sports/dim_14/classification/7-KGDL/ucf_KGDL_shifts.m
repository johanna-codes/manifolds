...
for s=1:length(vec_shift)
     ...
     acc = [];
     load_sub_path_test =strcat(path, 'dim_', int2str(dim), '/grass_points/one-grass-point/scale', num2str(scale_factor), '-shift', int2str(shift_test) );
    ...
    for video_ts= 1: n_videos %One Run
        ...
        if (~(strcmp(action_name,'Run-Side') && strcmp(folder_n,'001')))
            ...
            for video_tr=1: n_videos
                if (video_tr~=video_ts)
                    ...
                    if (~(strcmp(action_name_tr, 'Run-Side') && strcmp(folder_n_tr,'001')))
                        ...
                        %disp(tr);
                        name_load_gp = strcat( load_sub_path_train, '/grass_pt_', action_name_tr, '_', folder_n_tr, '_dim', int2str(dim), '_p', num2str(best_p), '.h5');
            ...
            labels_test = act_ts;
            %disp('Testing with: ');
            name_load_gp_ts = strcat( load_sub_path_test, '/grass_pt_', action_name_ts, '_', folder_n_ts, '_dim', int2str(dim), '_p', num2str(best_p), '.h5');
