function acc = LED_POLY_test(path,action_seq_names,scale_factor, shift, dim, n, num_videos )
...
j=1;
  for video_ts= 1: n_videos
      ...
      if (~(strcmp(action_name,'Run-Side') && strcmp(folder_n,'001')))
          load_svm_model = strcat( './svm_models/LED_POLY_run_', int2str(video_ts), '_n', num2str(n),'.mat');
     ...
     save_labels = strcat('./svm_results/LED_POLY_scale', int2str(scale_factor), '-shift', int2str(shift),'-n',num2str(n),'.mat' );     
