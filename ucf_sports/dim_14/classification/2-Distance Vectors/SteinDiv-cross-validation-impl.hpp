...
inline
void
cv_dist_vector_SteinDiv::train(int tr_scale, int tr_shift )
{
...
inline
void
cv_dist_vector_SteinDiv::svm_train()
{
...
inline
void
cv_dist_vector_SteinDiv::test(int ts_scale, int ts_shift)
{
  ...
  for (int test_i = 0; test_i< action_seq_names.n_rows; ++test_i)
  {
    ...
    if (!(action_name=="Run-Side" && folder_n=="001"))
    {
      ...
      load_svm_model << "./svm_models/SteinDiv_run_" << test_i+1;
	...
	acc++;
      ...
      main_save << "./svm_results/SteinDiv_scale" <<  ts_scale << "-shift"<< ts_shift;
...
inline
void
cv_dist_vector_SteinDiv::distances(int scale_factor, int shift)
{
  ...
  for (int test_i = 0; test_i< action_seq_names.n_rows; ++test_i)
  {
    ...
    if (!(action_name=="Run-Side" && folder_n=="001"))
    {
      ...
      save_vec_dist << "./SteinDiv/dist_vector_" << action_name << "_" <<  folder_n << "_dim" << dim  << ".h5";
...
inline
vec
cv_dist_vector_SteinDiv::dist_one_video(field <std::string> action_seq_names, int test_i, std::string load_sub_path, std::string load_cov)
{
