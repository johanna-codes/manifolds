function acc =  kth_test_LED_POLY_hor_ver(path,scale_factor, shift, dim, n, flag_shift)
...
if (flag_shift == true)
load_sub_path =strcat(path, 'cov_matrices/kth-one-cov-mat-dim', int2str(dim), '/sc', int2str(sc), '/scale', num2str(scale_factor), '-horshift', int2str(shift) );
end
if (flag_shift == false)
load_sub_path =strcat(path, 'cov_matrices/kth-one-cov-mat-dim', int2str(dim), '/sc', int2str(sc), '/scale', num2str(scale_factor), '-vershift', int2str(shift) );
end
