...
):path(in_path), actionNames(in_actionNames), all_people (in_all_people), scale_factor(in_scale_factor), shift(in_shift), total_scenes(in_scene), dim(in_dim),flag_shift(in_flag_shift)
{
...
kth_cv_omp::logEucl()
{
  ...
  {
    load_sub_path  << path << "cov_matrices/kth-one-cov-mat-dim" << dim << "/sc" << sc << "/scale" << scale_factor <<  "-horshift"<< shift ;
    
  }
  
  if (!flag_shift)//Vertical Shift
  {
    load_sub_path  << path << "cov_matrices/kth-one-cov-mat-dim" << dim << "/sc" << sc << "/scale" << scale_factor << "-vershift"<< shift ;
  }
...
kth_cv_omp::proj_grass(int in_p)
{
  ...
  if (flag_shift) //Horizontal Shift
  {
    load_sub_path << path << "grass_points/kth-grass-point-one-dim" << dim << "/sc" << sc << "/scale" << scale_factor << "-horshift"<< shift ;
    
  }
  
  if (!flag_shift)//Vertical Shift
  {
    load_sub_path << path << "grass_points/kth-grass-point-one-dim" << dim << "/sc" << sc << "/scale" << scale_factor << "-vershift"<< shift ;
  }
