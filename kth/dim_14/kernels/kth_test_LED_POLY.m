function acc = kth_test_LED_POLY(path,scale_factor, shift, dim, n, d)
...
j=1;
  for pe_ts= 1: n_peo
      ...
      load_svm_model =strcat( './svm_models_LED_POLY/LED-POLY_svm_run_',num2str(pe_ts), '_n', num2str(n), '_degree', num2str(d), '.mat');
      ...
      for act_ts = 1:n_actions
          ...
          K_test = compute_poly_kernel_svm(X_test,X_train, LED_POLY_KERNEL,gamma,d);
     ...
     save_labels = strcat('./svm_results_LED_POLY/LED-POLY_scale', int2str(scale_factor), '-shift', int2str(shift),'-n',num2str(n),'-degree',num2str(d),'.mat' );     
