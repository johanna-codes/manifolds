...
get_GMM_all_vecs( field<string> all_people, int N_cent, int num_SPD )
{
 ...
 all_people = in_all_people;
  ...
  for (int pe = 0; pe< n_peo; ++pe)
  {
	...
	std::ostringstream load_spd_vec;
      load_spd_vec << "./vec_spd/sc" << sc << "/scale" << scale_factor << "-shift"<< shift << "/vecSPD_" << i << "_out_" << num_SPD << "_" <<all_people (pe) << "_" << actions(act) <<  ".h5";
      vecSPD_cov.load(save_spd_vec.str(), hdf5_binary);
      all_vectors.col(k)=vecSPD_cov;
      k++;
